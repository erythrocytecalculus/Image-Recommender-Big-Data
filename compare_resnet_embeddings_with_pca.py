...
# Load embeddings from a file in batches
def load_embeddings_in_batches(file_path, batch_size):
    with open(file_path, "rb") as f:
        data = pickle.load(f)  # Deserialize the data from the pickle file
...
def compute_embedding(img_path, model):
    ...
    img_tensor = preprocess(img).unsqueeze(0)  # Preprocess the image and add batch dimension
    with torch.no_grad():
        embedding = model(img_tensor)  # Get the image embedding
    return embedding.view(-1).numpy()  # Flatten the embedding and return as a NumPy array

# Compute the embeddings for multiple images and average them
def compute_multiple_embeddings(img_paths, model):
    embeddings = [compute_embedding(img_path, model) for img_path in img_paths]  # List comprehension to compute embeddings
    return np.mean(embeddings, axis=0)  # Return the mean embedding
...
def find_top_similar_images(embedding, embeddings_batches, pca, top_n=5):
    embedding_pca = pca.transform([embedding])  # Apply PCA transformation
    all_similarities = []
    all_uuids = []

    # Compare the input embedding with the precomputed embeddings
    for uuids_batch, embeddings_batch in embeddings_batches:
        similarities = cosine_similarity(embedding_pca, embeddings_batch)[0]  # Calculate cosine similarities
        all_similarities.extend(similarities)
        all_uuids.extend(uuids_batch)

    # Sort similarities to find the top N most similar images
    all_similarities = np.array(all_similarities)
    top_indices = all_similarities.argsort()[-top_n:][::-1]  # Get the top N indices in descending order
    return [all_uuids[i] for i in top_indices]  # Return the UUIDs of the top similar images

# Load the image paths from the database based on UUIDs
def load_image_paths_from_db(db_path, uuids):
    conn = sqlite3.connect(db_path)  # Connect to the SQLite database
    cursor = conn.cursor()
    placeholders = ", ".join(["?"] * len(uuids))  # Create placeholders for SQL query
    query = f"SELECT uuid, file_path FROM images WHERE uuid IN ({placeholders})"
    cursor.execute(query, uuids)  # Execute the query
    rows = cursor.fetchall()  # Fetch the results
    conn.close()
    return {row[0]: row[1] for row in rows}  # Return a dictionary of UUID to file paths

# Plot the input images and their most similar images
def plot_images(main_image_paths, top_similar_images_paths):
    ...
    fig, axes = plt.subplots(2, num_columns, figsize=(20, 10))  # Create a subplot grid

    # Plot the input images
    for i, img_path in enumerate(main_image_paths):
        img = cv2.cvtColor(cv2.imread(img_path), cv2.COLOR_BGR2RGB)  # Read and convert image to RGB
        axes[0, i].imshow(img)  # Display the image
        axes[0, i].set_title(f"Input Image {i+1}")  # Set the title
        axes[0, i].axis("off")  # Hide the axes

    # Plot the similar images
    for i, img_path in enumerate(top_similar_images_paths):
        img = cv2.cvtColor(cv2.imread(img_path), cv2.COLOR_BGR2RGB)  # Read and convert image to RGB
        axes[1, i].imshow(img)  # Display the image
        axes[1, i].set_title(f"Similar Image {i+1}")  # Set the title
    ...
    plt.suptitle("Top Similar Images")  # Set the title for the entire figure
    plt.show()

# Main function for running the script
def main():
    input_image_paths = ["new_images/eye.jpg"]  # Paths to the input images
    pca_embeddings_path = "pca_embeddings_image_analysis_100.pkl"  # Path to PCA-reduced embeddings
    pca_model_path = "pca_model_image_analysis_100.pkl"  # Path to the saved PCA model
    database_path = "image_metadata.db"  # Path to the image database
    batch_size = 150000  # Batch size for loading embeddings

    # Load the pre-trained PCA model
    with open(pca_model_path, "rb") as f:
        pca = pickle.load(f)

    # Compute the embeddings for the input image(s)
    ...
    # Find the top similar images using the computed embedding
    ...
    # Get the file paths for the top similar images
    ...
    # Plot the input and similar images
