import os
import cv2
import sqlite3
import torch
import time
import pickle
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from PIL import Image
from sklearn.metrics.pairwise import cosine_similarity
from torchvision import models, transforms
import cProfile
import pstats


# Timer Start
start_time = time.time()

# Load the pre-trained ResNet model
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V1)
model = torch.nn.Sequential(*(list(model.children())[:-1]))  # Remove last layer
model.to(device)  # Move model to device
model.eval()

# Define image preprocessing pipeline
preprocess = transforms.Compose(
    [
        transforms.Resize(224),
        transforms.CenterCrop(224),
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    ]
)


def load_embeddings_in_batches(file_path, batch_size):
    with open(file_path, "rb") as f:
        data = pd.read_pickle(f)
    uuids = list(data.keys())
    embeddings = list(data.values())
    for i in range(0, len(embeddings), batch_size):
        yield uuids[i:i + batch_size], embeddings[i:i + batch_size]


def compute_embedding(img_path, model):
    img = Image.open(img_path).convert("RGB")
    img_tensor = preprocess(img).unsqueeze(0).to(device)
    with torch.no_grad():
        embedding = model(img_tensor)
    return embedding.view(-1).cpu().numpy()


def compute_multiple_embeddings(img_paths, model):
    batch_tensors = [preprocess(Image.open(img_path).convert("RGB")).unsqueeze(0) for img_path in img_paths]
    batch_tensor = torch.stack(batch_tensors).to(device)
    with torch.no_grad():
        embeddings = model(batch_tensor)
    return np.mean(embeddings.view(len(img_paths), -1).cpu().numpy(), axis=0)


def find_top_similar_images(embedding, embeddings_batches, top_n=5):
    all_similarities = []
    all_uuids = []

    for uuids_batch, embeddings_batch in embeddings_batches:
        similarities = cosine_similarity([embedding], np.array(embeddings_batch))[0]
        all_similarities.extend(similarities)
        all_uuids.extend(uuids_batch)

    top_indices = np.argsort(all_similarities)[-top_n:][::-1]
    return [all_uuids[i] for i in top_indices]


def load_image_paths_from_db(db_path, uuids):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    placeholders = ", ".join(["?"] * len(uuids))
    query = f"SELECT uuid, file_path FROM images WHERE uuid IN ({placeholders})"
    cursor.execute(query, uuids)
    rows = cursor.fetchall()
    conn.close()
    return {row[0]: row[1] for row in rows}


def plot_images(main_image_paths, top_similar_images_paths):
    plt.figure(figsize=(20, 10))
    
    for i, img_path in enumerate(main_image_paths):
        img = cv2.imread(img_path)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        plt.subplot(2, len(main_image_paths), i + 1)
        plt.imshow(img)
        plt.title(f"Input Image {i+1}")
        plt.axis("off")

    for i, img_path in enumerate(top_similar_images_paths):
        img = cv2.imread(img_path)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        plt.subplot(2, len(main_image_paths), len(main_image_paths) + i + 1)
        plt.imshow(img)
        plt.title(f"Similar Image {i+1}")
        plt.axis("off")

    plt.show()


def main():
    # Example usage
    input_images = ["new_images/eye.jpg"]
    cosine_similarities_path = "combined_embeddings.pkl"
    database_path = "image_metadata.db"
    batch_size = 150000  # Adjust batch size depending on system memory
    multiple_inputs = len(input_images) > 1

    # Timing the computation process
    compute_start_time = time.time()

    if multiple_inputs:
        print(f"Computing embeddings for multiple images: {input_images}")
        new_image_embedding = compute_multiple_embeddings(input_images, model)
    else:
        print(f"Computing embedding for a single image: {input_images[0]}")
        new_image_embedding = compute_embedding(input_images[0], model)

    print(f"Time taken for embedding computation: {time.time() - compute_start_time} seconds")

    # Find top similar images from precomputed dataset embeddings
    print("Finding top similar images...")
    find_start_time = time.time()

    embeddings_batches = load_embeddings_in_batches(cosine_similarities_path, batch_size)
    top_similar_images = find_top_similar_images(new_image_embedding, embeddings_batches)

    print(f"Top similar images: {top_similar_images}")
    print(f"Time taken for finding similar images: {time.time() - find_start_time} seconds")

    # Fetch image paths for similar images from the database
    print("Loading image paths from the database...")
    load_start_time = time.time()

    image_paths_dict = load_image_paths_from_db(database_path, top_similar_images)

    print(f"Image paths loaded: {image_paths_dict}")
    print(f"Time taken for loading image paths: {time.time() - load_start_time} seconds")

    # Retrieve the file paths for the top similar images
    top_similar_images_paths = [image_paths_dict[uuid] for uuid in top_similar_images]

    # Plot the images
    print("Plotting images...")
    plot_images(input_images, top_similar_images_paths)

    print("Images plotted.")

if __name__ == "__main__":
    profiler = cProfile.Profile()
    profiler.enable()

    main()

    profiler.disable()
    stats = pstats.Stats(profiler).sort_stats("cumtime")
    stats.print_stats(10)  # Display top 10 functions by cumulative time

    # Final timing print
    print(f"Total execution time: {time.time() - start_time} seconds")